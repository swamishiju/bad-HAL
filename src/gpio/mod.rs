...
impl GpioReg {
    /// Issues a GPIO reset via GPRCM.
    pub fn reset(&mut self) {
        const RSTCTL_KEY_UNLOCK: u32 = 0xB100_0000;
        const RSTCTL_STICKY_CLEAR: u32 = 0x0000_0002;
        const RSTCTL_ASSERT: u32 = 0x0000_0001;

        self.gprcm.rstctl = RSTCTL_KEY_UNLOCK | RSTCTL_STICKY_CLEAR | RSTCTL_ASSERT;
    }

    /// Enables GPIO power domain.
    ...
    /// Enables output for the specified pins.
    pub fn enable_output(&mut self, pins: u32) {
        self.doeset31_0 = pins;
    }

    /// Toggles the specified pins.
    pub fn toggle(&mut self, pins: u32) {
        self.douttgl31_0 = pins;
    }
}

/// Initializes all GPIO ports (A, B, C) and enables power to each.
pub fn gpio_init() -> (
    &'static mut GpioReg,
    &'static mut GpioReg,
    &'static mut GpioReg,
) {
    let gpio_a = GpioReg::from_addr(0x400A_0000);
    let gpio_b = GpioReg::from_addr(0x400A_2000);
    let gpio_c = GpioReg::from_addr(0x400A_4000);
